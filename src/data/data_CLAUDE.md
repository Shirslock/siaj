...
|---------|-----------|-----------------|
| `catalogos.ts` | Todos los catálogos de dropdowns con IDs oficiales | Tablas Codificadoras Excel |
| `formularios.ts` | Campos por tipo de gestión (etapa mesa + abogado) | FORMULARIOS.md |
...
| `INT_` | Roles de interviniente |
| `TC_`  | Tipos de documento de interviniente |

---

## Asignación de letrados (regla de negocio)

**Civil/Laboral → FIFO:**
```ts
import { getAbogadosFifo } from './usuarios'
const abogados = getAbogadosFifo('CIVIL')  // ordenados por fifoOrder
```
...
Mapa completo línea → abogado titular:
| Línea | Abogado | ID |
|-------|---------|-----|
...
Estos 5 campos aparecen en TODOS los tipos de gestión al dar de alta:
...
**NO van aquí:** Carátula ni N° Causa (son campos fijos del expediente, no del formulario dinámico).
...
- OFICIO en área PENAL → usar `form.variante_penal` en lugar de `form.mesa/abogado`
- Campos con `dependsOn`: ocultos por defecto, mostrar si condición se cumple
