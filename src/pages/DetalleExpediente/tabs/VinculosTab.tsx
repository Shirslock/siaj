                ...
                <div key={v.id} className="bg-white rounded-2xl shadow-card p-4 flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className="font-mono text-sm font-bold text-[#242C4F]">{v.id}</span>
                      <AreaBadge area={v.area} />
                      <EstadoBadge code={v.estado} label={v.estadoLabel} />
                      <span className="text-[10px] font-bold uppercase tracking-wider text-[#758A93] bg-[#E3E4E9] px-2 py-0.5 rounded-full">
                        {v.tipo_relacion.replace(/_/g, ' ')}
                      </span>
                    </div>
                    <p className="text-sm text-[#242C4F] truncate">{v.caratula}</p>
                    <div className="flex items-center gap-3 mt-1 text-xs text-[#758A93]">
                  ...
                  <button
                    ...
                    title="Desvincular"
                    className="p-1.5 rounded-lg text-[#758A93] hover:bg-red-50 hover:text-red-600 transition-colors flex-shrink-0"
                  >
              ...
              <button
                ...
                className={`w-full text-left px-4 py-2.5 rounded-xl border transition-all ${
                  seleccionado?.id === e.id
                    ? 'border-[#242C4F] bg-[#E4EDF2]'
                    : 'border-[rgba(0,0,0,0.10)] hover:bg-[#E3E4E9]'
                }`}
              >
                ...
                <p className="text-xs text-[#242C4F] truncate">{e.caratula}</p>
              </button>
            ))}
            {filtrados.length === 0 && (
              <p className="text-sm text-center text-[#758A93] py-4">Sin resultados.</p>
            )}
          </div>
      ...
      >
        {confirmarDesvincular && (
          <div className="space-y-2">
            ...
            <div className="bg-[#EEEBE6] rounded-xl px-4 py-3 mt-2">
              <p className="font-mono text-xs font-bold text-[#242C4F]">{confirmarDesvincular.id}</p>
              <p className="text-xs text-[#758A93] mt-0.5 truncate">{confirmarDesvincular.caratula}</p>
            </div>
          </div>
        )}
      </Modal>
